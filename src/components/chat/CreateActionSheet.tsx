    ...
    <Drawer open={open} onOpenChange={handleOpenChange}>
      <DrawerContent style={{ backgroundColor: "hsl(var(--color-surface))" }} className="rounded-t-[20px]">
        ...
        <div className="px-5 pb-6">
          {mode === "choose" && (
            <div className="space-y-2">
              <button onClick={() => setMode("plan")}
                className="w-full flex items-center gap-3 p-3.5 rounded-lg text-left transition-colors hover:opacity-90"
                style={{ backgroundColor: "hsl(var(--color-surface-card))" }}>
                <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ backgroundColor: "hsl(var(--color-surface-sage))" }}>
              ...
              <button onClick={() => setMode("poll")}
                className="w-full flex items-center gap-3 p-3.5 rounded-lg text-left transition-colors hover:opacity-90"
                style={{ backgroundColor: "hsl(var(--color-surface-card))" }}>
                <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ backgroundColor: "hsl(var(--color-surface-raised))" }}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={question} onChange={(e) => setQuestion(e.target.value)}
                  placeholder="Vad vill du fråga?" className="w-full px-3 py-2.5 text-[13px] rounded-lg border outline-none" style={inputStyle} />
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={planTitle} onChange={(e) => setPlanTitle(e.target.value)}
                  placeholder="T.ex. Middag, Fika, Vandring..." className="w-full px-3 py-2.5 text-[13px] rounded-lg border outline-none" style={inputStyle} />
              </div>
              <div>
                ...
                <input value={planDate} onChange={(e) => setPlanDate(e.target.value)}
                  placeholder="T.ex. Torsdag kväll, Nästa helg..." className="w-full px-3 py-2.5 text-[13px] rounded-lg border outline-none" style={inputStyle} />
              </div>
              <div>
                ...
                <input value={planLocation} onChange={(e) => setPlanLocation(e.target.value)}
                  placeholder="T.ex. Hemma hos mig, Café..." className="w-full px-3 py-2.5 text-[13px] rounded-lg border outline-none" style={inputStyle} />
              </div>
          ...
          {mode !== "choose" && (
            <button onClick={() => setMode("choose")} className="w-full mt-2 py-2 text-[12px] font-medium rounded-lg" style={{ color: "hsl(var(--color-text-secondary))" }}>
              ← Tillbaka
            </button>
          )}
        </div>
