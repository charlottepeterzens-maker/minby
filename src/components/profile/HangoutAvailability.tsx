...
const HangoutAvailability = ({ userId, isOwner }: Props) => {
  ...
  const renderEntryRow = (entry: AvailabilityEntry) => {
      ...
      <div key={entry.id} className="relative">
        {editingEntryId === entry.id && isOwner ? (
            ...
            <div className="flex flex-wrap gap-2">
              {ACTIVITY_OPTIONS.map((opt) => (
                <button
                  ...
                  style={{
                    borderRadius: 20, fontSize: 13, padding: "6px 14px",
          ...
          <button
            ...
            className={cn(
              "w-full flex items-center bg-card rounded-[16px] border-[0.5px] border-border p-2.5 text-left transition-all active:scale-[0.98]",
            ...
            <div className="flex-1 min-w-0 pr-5">
              ...
              {(entry.activities.length > 0 || entry.custom_note) && (
                <div className="flex items-center flex-wrap gap-1 mt-1">
                  ...
                  {entry.activities.length > 0 && entry.custom_note && <span className="text-[10px]" style={{ color: "#7A6A85" }}>·</span>}
                  {entry.custom_note && <span className="text-[12px] truncate" style={{ color: "#7A6A85" }}>{entry.custom_note}</span>}
                </div>
              )}
            </div>
            {!isOwner && (
    ...
    <div className="bg-card border border-border/50 rounded-lg p-4">
      ...
      {/* Entries split into confirmed and available */}
      {entries.length === 0 ?
      ...
      <div className="space-y-4">
          {/* Confirmed dates */}
          {(() => {
            const confirmed = entries.filter(e => e.entry_type === 'confirmed');
            if (confirmed.length === 0 && !isOwner) return null;
            return (
              <div>
                ...
                <div className="space-y-2">
                  ...
                  ) : confirmed.map((entry) => renderEntryRow(entry))}
                </div>
              </div>
            );
          })()}
          ...
          {/* Available dates */}
          {(() => {
            const available = entries.filter(e => e.entry_type !== 'confirmed');
            if (available.length === 0 && !isOwner) return null;
            return (
              <div>
                <p className="text-[11px] font-medium uppercase tracking-wide mb-2" style={{ color: "#7A6A85" }}>
                  {t("hangoutAvailableLabel")}
                </p>
                <div className="space-y-2">
                  {available.length === 0 ? (
                    <p className="text-[12px] text-muted-foreground py-1">{t("hangoutNoAvailable")}</p>
                  ) : available.map((entry) => renderEntryRow(entry))}
                </div>
              </div>
            );
          })()}
